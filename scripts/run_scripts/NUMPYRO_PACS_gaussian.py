...
ids = fcat['id']
PSW_mu = fcat['PSW'] # * hasMIPS
PSW_sigma = fcat['PSW_err'] * 2
PMW_mu = fcat['PMW'] # * hasMIPS
...
prior100.prior_cat(inra,indec,prior_cat,flux_mu=PSW_mu[sgood],flux_sigma=PSW_sigma[sgood],ID=ids[sgood]) #Set input catalogue
prior100.prior_bkg(bkg100,5) #Set prior on background
#---prior160--------
prior160=xidplus.prior(im160,nim160,im160phdu,im160hdu)
prior160.prior_cat(inra,indec,prior_cat,flux_mu=PMW_mu[sgood],flux_sigma=PMW_sigma[sgood],ID=ids[sgood])
...
fit=PACS.all_bands_gaussian(prior100,prior160,num_samples=750,num_warmup=750)
posterior=xidplus.posterior_numpyro(fit,[prior100,prior160])
#Save output
outfile=output_folder+'PACS_gaussian_'+str(tiles[taskid-1])+'_'+str(order)+'.pkl'
with open(outfile, 'wb') as f:
