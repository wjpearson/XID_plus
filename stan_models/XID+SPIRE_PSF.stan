...
data {
  ...
  int Val_psw[nnz_psw];//non neg values in image matrix
  ...
  int<lower=1> N_psf;//number of points in psf
  vector[N_psf] pind; //1D psf grid (1 arcsec res)
...
parameters {
  ...
  vector[N_psf] PSF;//1D radial profile of PSF
...
model {
  ...
  //GP prior on PSF
  PSF ~ multi_normal(mu,Sigma);
  //Put prior on 1st value so it peaks at 1
  PSF[1] ~normal(1.0,0.01);
